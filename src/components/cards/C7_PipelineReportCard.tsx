    ...
    <div className="relative bg-neutral-900 rounded-xl border border-neutral-800 hover:border-neutral-700 transition-colors overflow-hidden">
      ...
      <div className="px-4 pb-3">
        <div className="grid grid-cols-5 gap-2">
          <MetricItem label="开放岗位" value={m.open_positions} unit="个" color="text-violet-400" />
          <MetricItem label="活跃候选人" value={m.active_candidates} unit="人" color="text-blue-400" />
          <MetricItem label="本期入职" value={m.hired_this_period} unit="人" color="text-emerald-400" />
          <MetricItem label="平均周期" value={m.avg_time_to_hire_days} unit="天" color="text-amber-400" />
          <MetricItem label="接受率" value={m.offer_accept_rate} unit="%" color="text-emerald-400" />
        </div>
      </div>
          ...
          {f.map((s, i) => (
            <div key={i} className="flex items-center gap-3">
              ...
              <div className="flex-1 h-4 bg-neutral-800 rounded overflow-hidden">
                <div
                  className="h-full rounded bg-gradient-to-r from-violet-400/40 to-violet-400/70 transition-all"
                  style={{ width: `${Math.min(100, s.conversion_rate * 100)}%` }}
                />
              </div>
            ...
            {a.map((alert, i) => (
              <div key={i} className="flex items-start gap-2">
                <span className={cn(
                  ...
                  alert.status === 'stuck' ? 'bg-red-400' : 'bg-amber-400',
                )} />
                <div>
                  <p className="text-xs text-neutral-300">{alert.title}</p>
                  <p className="text-[10px] text-neutral-500">{alert.reason}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="p-3 rounded-lg bg-violet-500/5 border border-violet-500/10 space-y-1.5">
            {ins.map((insight, i) => (
              <p key={i} className="text-xs text-violet-200/80 leading-relaxed flex items-start gap-2">
                <TrendingDown className="w-3 h-3 mt-0.5 flex-shrink-0 text-violet-400" />
                {insight}
              </p>
            ))}
          </div>
...
function MetricItem({ label, value, unit, color }: { label: string; value: number; unit: string; color: string }) {
  return (
    <div className="text-center p-2 rounded-lg bg-neutral-800/50">
      <p className={cn('text-lg font-bold', color)}>{value}<span className="text-xs font-normal text-neutral-600 ml-0.5">{unit}</span></p>
