            ...
            <div className="px-3 pb-3 space-y-2">
              {category.questions.map((q, j) => (
                <motion.div
                  ...
                  className="pl-3 border-l-2 border-neutral-700"
                >
...
function DifficultyBadge({ level }: { level: string }) {
  const map: Record<string, string> = { easy: 'bg-neutral-700 text-neutral-400', medium: 'bg-amber-500/10 text-amber-400', hard: 'bg-red-500/10 text-red-400' };
  const label: Record<string, string> = { easy: '简单', medium: '中等', hard: '困难' };
  return <span className={cn('text-[10px] px-1 py-0.5 rounded flex-shrink-0', map[level] ?? map.medium)}>{label[level] ?? level}</span>;
}
